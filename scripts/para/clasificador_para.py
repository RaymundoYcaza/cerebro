...
def quick_classification(source: Path, para_root: Path, vault_path: Path, config: Dict) -> Classification:
    ...
    resource_type = choose_from_list("Tipo de recurso", sorted(set(list(EXTENSION_TYPE_MAP.values()) + ["carpeta", "documento"])), default_index=0 if resource_type not in sorted(set(list(EXTENSION_TYPE_MAP.values()) + ["carpeta", "documento"])) else sorted(set(list(EXTENSION_TYPE_MAP.values()) + ["carpeta", "documento"])).index(resource_type))
    category = choose_from_list("Categoria curada en zResources", categories, default_index=default_index)
    subcategory = ui.input("Subcategoria", default="")
...
def elegir_imagen_cercana(
    ...
    """
    Busca imagenes en la misma carpeta del archivo/carpeta procesado.
    Permite elegir una, la copia dentro del vault y devuelve el wikilink para YAML.
    """
